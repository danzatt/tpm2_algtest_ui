...
INFO_MESSAGE = \
"""
This tool makes performance tests and collects information about
Trusted Platform Module (TPM) 2.0 cryptographic chip. The collected
data doesn't contain any personal information and is completely
anonymised.

All received results will be collected into one dataset which
will be released publicly. This enables developers gain insight
into capabilities of TPMs and potentially discover security flaws.
...
for research purposes regarding the performance and the security of the ke
generated by your TPM.

The test might take 1-4 hours. After it finishes you can choose to upload
the results into Masaryk University information system or store it on a USB drive
and send it later.

We may need to contact you in the future if we need to perform more tests or
get further info about your system. If you choose to type your email address,
it will be included in the collected data. The email address won't be shared
with anybody and you will not receive any messages unrelated to the TPM research.
"""
...
class AlgtestTestRunner(Thread):
    ...
    def store_results(self, store_type):
        ...
        if os.path.isdir(RESULT_PATH):
            try:
                ...
                with open(os.path.join(RESULT_PATH, "README.txt"), "w") as readme_file:
                    readme_file.write(INFO_MESSAGE)
            except:
    ...
    def run_quicktest(self):
        ...
        output = result.stdout.decode("ascii").replace("\t", "").split("\n")
...
class TPM2AlgtestUI:
    ...
    def popup_info_show(self):
        ...
        self.popup_info_hide_button = YUI.widgetFactory().createPushButton(popup_vbox, "&Hide")
